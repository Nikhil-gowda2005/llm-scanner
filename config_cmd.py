...
Commands:
    ...
    llm-scanner-config set-google-key <key>   (Unused - kept for backward compatibility)
...
Examples:
    ...
    llm-scanner-config set-google-key AIza_xyz789
...
def cmd_set_groq_key(key: str) -> None:
    ...
    print()
    print("The LLM Judge panel (GPT-OSS 120B + 27B + 20B) is now enabled.")
    print("Run your scan without --groq-key:")
    print("  llm-scanner --target http://localhost:5000 --apikey demo123")


def cmd_set_google_key(key: str) -> None:
    """Saves the Google AI Studio API key (unused — kept for backward compatibility)."""
    key = key.strip()
    if not key:
        print("[ERROR] Key cannot be empty.")
        sys.exit(1)
    cfg = _load_config()
    cfg["google_ai_api_key"] = key
    _save_config(cfg)
    print(f"[OK]  Google AI API key saved: {_mask(key)}")
    print(f"      Stored in: {CONFIG_FILE}")
    print()
    print("Google AI key saved (note: all judges now use Groq GPT-OSS models).")
    print("Get a free key at: https://aistudio.google.com")
    print("Run with multi-judge enabled:")
    print("  llm-scanner --target http://localhost:5000 --judge-mode consensus")
...
def cmd_show() -> None:
    ...
    print(f"  Google AI API key : {_mask(google_key)}")
    print(f"  Chatbot API key   : {_mask(scanner_key)}")
    print()

    if not groq_key:
        print("  [TIP] Add your Groq key to enable the LLM Judge panel (GPT-OSS 120B):")
        print("        llm-scanner-config set-groq-key gsk_your_key_here")
        print("        Get a free key at: https://console.groq.com")
    else:
        print("  [OK] LLM Judge panel (GPT-OSS 120B + 27B + 20B + Superior) is enabled.")
        print()
        print("  Multi-judge panel: 4 judges ready:")
        print("    - Judge 1: GPT-OSS 120B (Groq)")
        print("    - Judge 2: GPT-OSS 27B  (Groq)")
        print("    - Judge 3: GPT-OSS 20B  (Groq)")
        print("    - Superior Judge: GPT-OSS 120B (Groq)")
...
def load_saved_config() -> dict:
    ...
    Priority order for each key:
        1. CLI flag (--groq-key / --google-ai-key / --apikey)
        2. Environment variable (GROQ_API_KEY / GOOGLE_AI_API_KEY / LLM_SCANNER_API_KEY)
        3. Saved config file (~/.llm-scanner/config.json)  <- this function

    Returns:
        dict with keys: "groq_api_key", "google_ai_api_key", "llm_scanner_api_key"
    """
    return _load_config()
...
def main() -> None:
    ...
    p_groq = subparsers.add_parser(
        "set-groq-key",
        help="Save your Groq API key for the LLM Judge panel (GPT-OSS 120B).",
    )
    p_groq.add_argument("key", help="Your Groq API key (starts with gsk_...)")

    # set-google-key
    p_google = subparsers.add_parser(
        "set-google-key",
        help="(Unused) Google AI Studio API key — kept for backward compatibility.",
    )
    p_google.add_argument("key", help="Your Google AI Studio API key (starts with AIza...)")
    ...
    elif args.command == "set-google-key":
        cmd_set_google_key(args.key)
